...
//如果-o - 则达标输出到命令行
  ...
  if (binary_mode) {
//如果是binary_mode的话直接把共享内存段直接写到输出文件中
...
//利用桶对hit进行分类，此时trace_bits就是桶的个数
  classify_counts(trace_bits, binary_mode ?
                  count_class_binary : count_class_human);

  if (!quiet_mode)
    SAYF(cRST "-- Program output ends --\n");
//如果未超时且退出码有问题则表示产生了crash
