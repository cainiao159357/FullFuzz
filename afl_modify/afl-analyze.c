...
static u32 run_target(char** argv, u8* mem, u32 len, u8 first_run) {
  ...
  MEM_BARRIER();
//把in_data写入prog_in_fd中,这相当于覆盖了input_file的内容，那要input_file的内容干啥呢
...
//另外，对于每个位置，如果对它的修改造成的影响，与修改前一个字节完全不一样，则认为它与前一个字节分别属于不同的 field。
// 这是有道理的，例如 uint32 magic number 的第二、第三个字节被修改后的行为，肯定是完全一样的。而当前后两个字节分属不同的 field 时，
// 对它们的修改大概率会产生完全不同的效果。
    if (prev_xff != xor_ff && prev_x01 != xor_01 &&
        prev_s10 != sub_10 && prev_a10 != add_10) seq_byte ^= 0x80;

//相同的field有相同的最高位，比如说有相同的1或者0
    b_data[i] |= seq_byte;
