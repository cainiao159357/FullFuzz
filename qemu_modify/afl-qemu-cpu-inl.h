...
//
  ...
  if (!afl_area_ptr) {
    printf("[*] In forkserver not find afl_area_ptr\n");
    return;
  }
...
void afl_setup(void) {
  ...
  if (id_str) {
    ...
    printf("not get shm id\n");
    // exit(-1);
  }
  //设置读端非阻塞
  int flags = fcntl(FORKSRV_FD, F_GETFL);
  if (flags == -1) {
    perror("fcntl(F_GETFL)");
    return 1;
  }
  flags |= O_NONBLOCK;
  if (fcntl(FORKSRV_FD, F_SETFL, flags) == -1) {
    perror("fcntl(F_SETFL)");
    return 1;
  }
  if (write(FORKSRV_FD + 1, temp, 4) != 4) return;
