...
static void traverse_task_struct_add(CPUState *env)
{
   ...
   // uint32_t task_pid = 0;
	...
	//DECAF_read_ptr(env, mm + OFFSET_PROFILE.mm_pgd, &task_pgd);
        ...
        {
            // CPUArchState *arch=(CPUArchState *)env->env_ptr;
// #ifdef TARGET_MIPS
//             target_ulong task = arch->active_tc.gpr[4];
//             printf("task_addr:0x%lx\n",task);
//             char pro_name[0x100];
//             BREAK_IF(DECAF_read_mem(env, task + OFFSET_PROFILE.ts_comm,
//                                     SIZEOF_COMM, pro_name) < 0);
//             printf("task_name:%s\n",pro_name);
// #endif
            // get task_pid
...
static void traverse_task_struct_remove(CPUState *env)
{
    ...
    while(true)
    {
        ...
        if (mm != 0)
        {
            ...
            // printf("task_addr:0x%lx\n",task);
            int _exit_code;
            BREAK_IF(DECAF_read_mem(env, task + OFFSET_PROFILE.ts_mm+12,
                                    SIZEOF_COMM, &_exit_code) < 0);
            BREAK_IF(DECAF_read_ptr(env, next_task + OFFSET_PROFILE.ts_tgid,
                                    &task_pid) < 0);
            // exit_code=_exit_code;
            // printf("task_exit_pid:%d\n",task_pid);
            unordered_map < uint32_t, process * >::iterator iter=process_pid_map.find(task_pid);
            if(iter!=process_pid_map.end()){
                iter->second->exit_code=_exit_code;
                VMI_remove_process(task_pid);
            }
#endif
        }
    ...
    //         // iter->second->exit_code=exit_code;
    //         // iter->second.
    //         break;
    //     }
    // }
